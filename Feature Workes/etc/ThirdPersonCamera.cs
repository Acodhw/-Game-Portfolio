...
public class ThirdPersonCamera : MonoBehaviour { 
    ...
    {
        RaycastHit rh;
        Physics.Raycast(transform.position, -transform.forward, out rh, 4f);
        if (rh.point != null && rh.collider.gameObject != player)
        {

            m_Camera.localPosition = new Vector3(0, 0, -rh.distance + 0.5f);

        }
        else
            m_Camera.localPosition = new Vector3(0, 0, -4);
    }
