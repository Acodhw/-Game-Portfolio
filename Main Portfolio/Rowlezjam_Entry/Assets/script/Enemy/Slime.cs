...
public class Slime : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (findPlayerTime > 0)
        {
            ...
            float tmp = 0;
            if (Mathf.Abs(player.position.x - transform.position.x) > PlayerFollowDistance || (transform.position.y - player.position.y < -2 || transform.position.y - player.position.y > 0))
            {
